...
class Text {
	...
	public static function censor($str, $badwords, $replacement = '#', $replace_partial_words = true)
	{
		...
		if (Utf8::strlen($replacement) == 1)
		{
			...
			$return = preg_replace_callback($regex, function($m) use ($replacement)
			{
	...
	protected static function _auto_link_urls_callback1($matches)
	{
		...
		$target = $host != Request::baseurl() ? ' target="_blank"' : '';

		return '<a href="' . $href . '" title="' . $href . '"' . $target . '>' . $href . '</a>';
	}
	...
	protected static function _auto_link_urls_callback2($matches)
	{
		...
		return '<a href="' . $href . '" title="' . $title . '"' . $target . '>' . $title . '</a>';
	}
	...
	{
		if(!in_array('browser', ['browser', 'version', 'robot', 'mobile', 'platform']))
		{
			return false;
		}
		...
		if ($value === 'browser' OR $value == 'version')
		{
			...
			// Load browsers
			$browsers = Config::fact('user_agents')->get('browser');
		...
		else
		{
			...
			$group = Config::fact('user_agents')->get($value);
