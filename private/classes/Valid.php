...
// Check if the email domain has a valid MX record
	...
	public static function credit_card($number, $type = NULL)
	{
		...
		$cards = Config::fact('credit_cards');

		// Check card type		
		$type = $cards->get(strtolower($type));
		...
		if (!in_array($length, preg_split('/\D+/', $type['length'])))
			return FALSE;

		// Check card number prefix
		if (!preg_match('/^' . $type['prefix'] . '/', $number))
			return FALSE;
	 ...
	 * @param   string  $match_label  for label of match
	 * @return  boolean
	 */
	public static function matches($array, $field, $match, $match_label)
	{
